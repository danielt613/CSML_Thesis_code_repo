# -*- coding: utf-8 -*-
"""
Original code from Hauptmann at https://github.com/asHauptmann/multiscale
"""
import astra
...
class Iteration(nn.Module):
    def __init__(self, op, op_adj,fbp):
        ...
        # TODO Work out where each layer goes
        # Find a way to dynamically change number of layers
    ...
    def forward(self, cur, curs,grads,fbps, y,it):
        ...
        if it < iter4Net-1:
            ...
            conv1 = self.dconv_down1(dx)
            x = self.maxpool(conv1) # scale 4->3
            ...
            x = self.maxpool(conv2) # scale 3->2
            ...
            x = self.maxpool(conv3) # scale 2->1
            ...
            x = self.maxpool(conv4)  # scale 1->0
            x = self.dconv_down5(x)
            x = self.xUp4(x)        # scale 0->1
            
            x = torch.cat([x, conv4], dim=1)
            x = self.dconv_up4(x)
            x = self.xUp3(x)        # scale 1->2
            
            x = torch.cat([x, conv3], dim=1)
            x = self.dconv_up3(x)
            x = self.xUp2(x)        # scale 2->3
            
            x = torch.cat([x, conv2], dim=1)      
            x = self.dconv_up2(x)
            x = self.xUp1(x)        # scale 3->4
